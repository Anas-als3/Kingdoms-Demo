...
public class DemoSeeder implements CommandLineRunner {
    ...
    private final PeriodScoreService periodScoreService;    // seasonal scores so seeded players show in the by-time leaderboards
    ...
    public void run(String... args) {
        try {
            ...
            Kingdom sports    = getOrCreateKingdom(KingdomType.SPORTS,       "مملكة الرياضة",   "حرّك جسدك — يتم التحقق عبر أنشطتك على Strava.");
            Kingdom charity   = getOrCreateKingdom(KingdomType.CHARITY,      "مملكة العطاء",   "اعطِ وردّ الجميل — يتم التحقق من تبرّع بنكي عبر Neotek Open Banking.");
            Kingdom volunteer = getOrCreateKingdom(KingdomType.VOLUNTEERING, "مملكة التطوع", "اخدم مجتمعك — يتم التحقق من شهادة PDF يفحصها الذكاء الاصطناعي.");
            Kingdom reading   = getOrCreateKingdom(KingdomType.READING,      "مملكة القراءة",   "اقرأ الكتب — يتم التحقق عبر اختبار قصير على WhatsApp (Google Books).");
            Kingdom gaming    = getOrCreateKingdom(KingdomType.GAMING,        "مملكة الألعاب",    "ارفع مستواك — يتم التحقق من وقت اللعب والإنجازات على Steam.");
            Kingdom faith     = getOrCreateKingdom(KingdomType.FAITH,         "مملكة الإسلاميات",     "ازدد قربًا — يتم التحقق عبر اختبار على WhatsApp حول محتوى القرآن.");
            Kingdom knowledge   = getOrCreateKingdom(KingdomType.KNOWLEDGE,   "مملكة المعرفة",   "تعلّم — يتم التحقق عبر اختبار معرفي بالذكاء الاصطناعي.");
            Kingdom nutrition   = getOrCreateKingdom(KingdomType.NUTRITION,   "مملكة التغذية",   "تغذَّ جيدًا — يتم التحقق من صورة وجبتك بالذكاء الاصطناعي.");
            Kingdom programming = getOrCreateKingdom(KingdomType.PROGRAMMING, "مملكة البرمجة", "برمِج — يتم التحقق من نشاطك على GitHub.");

            // 2b) one shared "first challenge" badge — awarded the first time a player finishes a challenge in ANY kingdom.
            seedFirstChallengeBadge(sports);
            ...
            Player p2 = seedHostPlayer("player2", "عبدالله", "player2.demo@kingdoms.local", "+966500000102", 30, sports, nutrition);
            Player p3 = seedHostPlayer("player3", "محمد", "player3.demo@kingdoms.local", "+966500000103", 25, sports);
            Player p4 = seedHostPlayer("player4", "سارة", "player4.demo@kingdoms.local", "+966500000104", 20, nutrition);
            Player p5 = seedHostPlayer("player5", "نورة", "player5.demo@kingdoms.local", "+966500000105", 15, sports);
    ...
    // One shared "first challenge" badge: type XP, requiredValue 1. The award query (findAwardableBadges) matches by
    // type + XP value only — NOT kingdom — so the first time a player finishes a challenge in a kingdom (earning XP),
    // they are granted this badge for that membership. So it naturally shows up under each kingdom they complete one in.
    private void seedFirstChallengeBadge(Kingdom kingdom) {
        if (!badgeRepository.findAll().isEmpty()) {
            return; // already seeded
        }
        Badge badge = new Badge();
        badge.setName("أول تحدٍّ مكتمل");
        badge.setDescription("أكمِل أول تحدٍّ لك في أي مملكة لتكسب هذا الوسام.");
        badge.setType(BadgeType.XP);
        badge.setRequiredValue(1);
        badge.setKingdom(kingdom); // non-null required: convertToOut reads badge.getKingdom().getName()
    ...
    private Player seedHostPlayer(String username, String displayName, String email, String phone, int xp, Kingdom... kingdoms) {
        ...
        for (Kingdom k : kingdoms) {
            seedMembership(p, k, xp);
        }
        seedPremiumSubscription(p);
        return p;
    }

    private void seedMembership(Player player, Kingdom kingdom, int totalXp) {
        ...
        membership.setTotalXP(totalXp);   // seeded XP so host players populate the division leaderboard (not 0s)
        membership.setDivision(3); // D3 — 10-30 XP is far below the next-division threshold, so they stay in D3 and division-matched lobbies still work.
        membership.setStreak(0);
        membership.setJoinedAt(LocalDateTime.now());
        membership = membershipRepository.save(membership);

        // Seasonal scores so the seeded players also populate the BY-TIME leaderboards (were empty). WEEKLY is what
        // the demo queries; MONTHLY is what the real finish-flow writes (ProgressRewardService) — seed both so either
        // window shows them. seasonalXp = totalXp so the by-time board matches the by-division board. Uses the same
        // addToPeriodScore() the live flow uses, so the period window is computed identically (current week/month).
        if (totalXp > 0) {
            periodScoreService.addToPeriodScore(membership, totalXp, Period.WEEKLY);
            periodScoreService.addToPeriodScore(membership, totalXp, Period.MONTHLY);
        }
    }
