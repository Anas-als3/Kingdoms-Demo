...
public class KingdomService {
    ...
    public KingdomRecommendationOut recommendKingdom(Integer playerId, KingdomRecommendationIn recommendationIN) {
        ...
        if (recommendationIN.getInterests() == null || recommendationIN.getInterests().isBlank()) {
            throw new ApiException("الاهتمامات مطلوبة");
        }
        ...
        try {
            ...
            throw new ApiException("نوع المملكة المقترح من الذكاء الاصطناعي غير صالح");
        }

        String reason = aiResult[1];

        Kingdom recommendedKingdom = availableKingdoms.stream().filter(kingdom -> kingdom.getType() == recommendedType).findFirst().orElseThrow(() -> new ApiException("اقترح الذكاء الاصطناعي مملكة غير صالحة"));
