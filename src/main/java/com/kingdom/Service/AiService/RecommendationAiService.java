...
public class RecommendationAiService {
    ...
    private String[] parseRecommendationJson(String json) {
        ...
        if (json == null || json.isBlank()) {
            throw new ApiException("أعاد الذكاء الاصطناعي استجابة فارغة");
        }
        ...
        if (!kingdomTypeMatcher.find()) {
            throw new ApiException("استجابة الذكاء الاصطناعي لا تحتوي على kingdomType");
        }

        if (!reasonMatcher.find()) {
            throw new ApiException("استجابة الذكاء الاصطناعي لا تحتوي على reason");
        }
