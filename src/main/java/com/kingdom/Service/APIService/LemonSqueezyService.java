...
public class LemonSqueezyService {
    ...
    public String createCheckout(Integer playerId, String plan) {
        ...
        if (player == null) {
            throw new ApiException("لم يتم العثور على اللاعب");
        }
    ...
    public void processWebhook(HttpHeaders headers, String rawBody) {
        try {
            ...
            e.printStackTrace();
            throw new ApiException("فشل معالجة الـ webhook: " + e.getMessage());
        }
    }
    ...
    private void handleSubscriptionCreatedOrUpdated(JsonNode root) {
        ...
        if (player == null) {
            throw new ApiException("لم يتم العثور على اللاعب بالمعرّف: " + playerId);
        }
    ...
    private void handleSubscriptionExpired(JsonNode root) {
        ...
        if (subscription == null) {
            throw new ApiException("لم يتم العثور على اشتراك للّاعب بالمعرّف: " + playerId);
        }
    ...
    private Integer extractPlayerId(JsonNode root) {
        ...
        if (playerIdNode.isMissingNode() || playerIdNode.asText().isBlank() || playerIdNode.asInt() == 0) {
            throw new ApiException("الحقل player_id مفقود في custom_data الخاص بـ webhook لـ Lemon Squeezy");
        }
    ...
    private String resolveVariantId(String plan) {
        ...
        throw new ApiException("خطة غير صالحة: يجب أن تكون MONTHLY أو ANNUAL");
    }
    ...
    private String extractCheckoutUrl(String response) {
        try {
            ...
            if (url == null || url.isBlank()) {
                throw new ApiException("لم يتم العثور على رابط الدفع في استجابة Lemon");
            }

            return url;

        } catch (Exception e) {
            throw new ApiException("فشل استخراج رابط الدفع: " + e.getMessage());
        }
    }
    ...
    private void verifySignature(String rawBody, String signature) {
        try {
            ...
            if (!computedSignature.equals(signature)) {
                throw new ApiException("توقيع webhook الخاص بـ Lemon Squeezy غير صالح");
            }

        } catch (Exception e) {
            throw new ApiException("فشل التحقق من التوقيع: " + e.getMessage());
        }
    }
    ...
    public String renewSubscription(Integer playerId, String plan) {
        Subscription subscription = subscriptionRepository.findSubscriptionById(playerId);
        if (subscription == null) {
            throw new ApiException("لا يوجد اشتراك حالي لتجديده");
        }
        if (subscription.getStatus() == SubscriptionStatus.ACTIVE && subscription.getExpiresAt() != null && subscription.getExpiresAt().isAfter(LocalDateTime.now().plusDays(7))) {
            throw new ApiException("الاشتراك ما زال نشطاً ولم يقترب موعد انتهائه");
        }
    ...
    private SubscriptionPlan determinePlanByVariantId(String variantId) {
        ...
        throw new ApiException("معرّف variant غير معروف لـ Lemon Squeezy: " + variantId);
    }
}