...
public class ChallengeQuestionWhatsappService {
    ...
    public void sendChallengeQuestions(Integer challengeProgressId) {
        ...
        if (progress == null) {
            throw new ApiException("لم يتم العثور على تقدم التحدي");
        }

        if (progress.getStatus() != ProgressStatus.JOINED) {
            throw new ApiException("يمكن بدء أسئلة واتساب للتحديات المنضمّ إليها فقط");
        }
        ...
        if (questions.isEmpty()) {
            throw new ApiException("لا توجد أسئلة لهذا التحدي");
        }
        ...
        if (!introSent) {
            throw new ApiException("فشل إرسال رسالة واتساب التعريفية");
        }
