...
public class RegistrationService {
    ...
    public Player register(UserIn req) {
        ...
        if (userRepository.findUserByUsername(username) != null) {
            throw new ApiException("اسم المستخدم هذا مستخدم بالفعل");
        }
        if (userRepository.findUserByPhoneNumber(phone) != null) {
            throw new ApiException("رقم الهاتف هذا مسجّل بالفعل");
        }
        for (User existing : userRepository.findAll()) {
            if (email.equalsIgnoreCase(existing.getEmail())) {
                throw new ApiException("البريد الإلكتروني هذا مسجّل بالفعل");
            }
        }
    ...
    private String hash(String password) {
        try {
            ...
            throw new ApiException("تعذّر تشفير كلمة المرور");
        }
    }
}
