...
public class OtpService {
    ...
    public void sendOtp(String phone) {
        ...
        if (key.isEmpty()) {
            throw new ApiException("رقم الهاتف مطلوب");
        }
    ...
    public void verifyOtp(String phone, String code) {
        ...
        if (stored == null) {
            throw new ApiException("لم يُطلب رمز لهذا الرقم — أرسل رمزاً أولاً");
        }
        if (System.currentTimeMillis() > stored.expiresAt()) {
            codesByPhone.remove(key);
            throw new ApiException("انتهت صلاحية الرمز — اطلب رمزاً جديداً");
        }
        if (!stored.value().equals(code == null ? "" : code.trim())) {
            throw new ApiException("الرمز غير صحيح");
        }
