...
public class StravaConnectService {
    ...
    public String authorizeUrlForPlayer(Integer playerId) {
        if (!stravaClient.isConfigured()) {
            throw new ApiException("خدمة Strava متوقفة أو غير مهيأة (اضبط strava.enabled و client-id/secret)");
        }
    ...
    public Map<String, Object> handleCallback(String code, String state, String error) {
        if (error != null && !error.isBlank()) {
            throw new ApiException("تم رفض تفويض Strava: " + error);
        }
        if (code == null || code.isBlank() || state == null || state.isBlank()) {
            throw new ApiException("رمز التفويض (code) أو الحالة (state) مفقود من Strava");
        }
        Integer playerId;
        try {
            playerId = Integer.valueOf(state.trim());
        } catch (NumberFormatException e) {
            throw new ApiException("قيمة الحالة (state) غير صالحة");
        }
        Player player = playerRepository.findPlayerById(playerId);
        if (player == null) {
            throw new ApiException("لم يُعثر على اللاعب لهذا التفويض");
        }

        JsonNode tokens = stravaClient.exchangeAuthorizationCode(code);
        if (tokens == null) {
            throw new ApiException("تعذّر استبدال رمز Strava (متوقف / غير مهيأ / انتهت صلاحية الرمز)");
        }
        String refresh = tokens.path("refresh_token").asText(null);
        if (refresh == null || refresh.isBlank()) {
            throw new ApiException("لم تُرجِع Strava رمز تحديث (refresh token)");
        }
        ...
        out.put("message", "تم ربط Strava. أكمل تحديات الرياضة وسيتم التحقق منها مقابل أنشطتك الخاصة.");
        return out;
    }
}
